...
class Parser:
	...
	def p_error(self, p):
		if p is None:
			...
			print("Erro sintático: '%s' na linha %d" % (p.value, p.lineno))
			#exit(1)

	

def print_arvore(node, level="-"):
	if node != None:
		if node.value != None:
...
def graph(node,w,i):
    if node != None:
        value = node.type + str(i)
        i = i + 1
        for son in node.child:
			w.edge(value,str(son) + str(i))
			graph(son,w,i)
...
if __name__ == '__main__':
	...
	try:
		...
		w = Digraph('G', filename='GraphOutput.gv.txt')
		graph(arvore.ast,w,i = 0)
		#Aw.view()
		#w.save(filename='GraphOutput.dot.txt')
		pass
	except Exception, e:
